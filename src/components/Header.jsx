...
const Header = ({ isDarkMode, onToggleDarkMode, totalGames, isSidebarOpen = false, onToggleSidebar, showSidebarHint = false, gridDensity = 'cozy', onChangeDensity, layoutMode = 'grid', onChangeLayout }) => {
  ...
  return (
    ...
    >
      ...
      {/* Sidebar toggle inside header: always on the right, with hint glow when closed */}
      <div className="relative">
        {/* Hint effect */}
        {!isSidebarOpen && showSidebarHint && (
          <>
            <span className="absolute -inset-1 rounded-full ring-2 ring-primary-400/60 dark:ring-primary-500/50 blur-[1px]" aria-hidden="true" />
            <span className="absolute -inset-2 rounded-full bg-primary-400/20 dark:bg-primary-500/20 animate-ping" aria-hidden="true" />
          </>
        )}
        <motion.button
          whileHover={{ scale: 1.05 }}
          ...
          className={`relative p-3 rounded-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors ${!isSidebarOpen && showSidebarHint ? 'shadow-[0_0_0_4px_rgba(59,130,246,0.15)]' : ''}`}
          aria-label={isSidebarOpen ? 'إغلاق القائمة الجانبية' : 'فتح القائمة الجانبية'}
          title={!isSidebarOpen && showSidebarHint ? 'افتح القائمة الجانبية' : undefined}
        >
          {isSidebarOpen ? (
      ...
      {createPortal(
        (
          ...
          <div className="mb-4">
            <h3 className="text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">عرض القائمة</h3>
            <div className="space-y-2">
              <button
                className={`w-full text-right px-3 py-2 rounded-md border ${layoutMode === 'grid' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white dark:bg-neutral-900 text-gray-800 dark:text-gray-100 border-gray-300 dark:border-neutral-700'}`}
              ...
              <button
                className={`w-full text-right px-3 py-2 rounded-md border ${layoutMode === 'list' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white dark:bg-neutral-900 text-gray-800 dark:text-gray-100 border-gray-300 dark:border-neutral-700'}`}
